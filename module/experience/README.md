...
Experience模块是一个独立的agent经验管理系统，基于topic动态构建和管理agent的经验。它与memory core解耦，专注于agent执行过程中的经验学习、存储和检索。

## 设计理念

### 核心原则

1. **经验独立于记忆**：经验系统独立于memory core，专注于agent执行经验
2. **基于Topic构建**：经验从topic生命周期中动态构建
3. **上下文感知**：经验检索考虑当前上下文，避免错误的经验迁移
4. **多维度索引**：支持按类型、任务、上下文、工具等多种维度检索
