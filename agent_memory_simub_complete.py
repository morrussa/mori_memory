...
def plot_health_dashboard(result: Dict[str, object], output_path: str = "health_dashboard.png",
                          figsize: Tuple[int, int] = (16, 10), dpi: int = 160) -> Optional[str]:
    """生成核心健康度仪表板
    
    4个核心健康度指标：
    1. Heat Gini Trend - 热度分布不均衡程度
    2. Recall Quality - 召回质量
    3. Memory Growth - 记忆增长趋势
    4. Compute Load Proxy - 计算负载代理
    
    Args:
        result: run_experiment() 返回的结果
        output_path: 输出路径
        figsize: 图表大小
        dpi: 分辨率
    ...
    if not snapshots:
        print("Warning: no snapshots to plot")
        return None
    ...
    def col(name: str) -> List[float]:
        return [float(r.get(name, 0.0)) for r in snapshots]
    ...
    recall_all = col("target_recall_all")
    ...
    cluster_count = col("cluster_count")
    ...
    preload_heated = col("preload_memories_heated")
    
    # ========== 创建图表 ==========
    fig, axes = plt.subplots(2, 2, figsize=figsize, constrained_layout=True)
    
    # 1. Heat Gini Trend - 热度分布恶化检测
    ax = axes[0, 0]
    ax.plot(turns, heat_gini, marker="o", linewidth=2.0, color="#2E86AB", label="heat_gini")
    ax.axhline(float(summary.get("heat_gini_start", heat_gini[0] if heat_gini else 0)), 
               color="#888888", linestyle="--", linewidth=1.4, label="start")
    ax.axhline(float(summary.get("heat_gini_end", heat_gini[-1] if heat_gini else 0)), 
               color="#E94F37", linestyle="--", linewidth=1.4, label="end")
    ax.set_title("Heat Gini Trend (Distribution Inequality)", fontsize=12, fontweight='bold')
    ax.set_xlabel("Turn")
    ax.set_ylabel("Gini Coefficient")
    ax.legend(loc="best")
    ax.grid(True, alpha=0.3)
    # 标注delta
    gini_delta = summary.get("heat_gini_delta", 0)
    ax.text(0.02, 0.98, f"Δ={gini_delta:+.4f}", transform=ax.transAxes, 
            fontsize=10, verticalalignment='top', 
            bbox=dict(boxstyle='round', facecolor='wheat', alpha=0.5))
    
    # 2. Recall Quality - 召回质量
    ax = axes[0, 1]
    ax.plot(turns, precision, marker="o", linewidth=2.0, label="precision@k", color="#1f77b4")
    ax.plot(turns, recall_recent, marker="s", linewidth=2.0, label="recall_recent", color="#2ca02c")
    ax.plot(turns, hit_rate, marker="^", linewidth=2.0, label="hit_rate", color="#17becf")
    ax.plot(turns, empty_target_rate, marker="o", linestyle="--", linewidth=1.8, 
            label="empty_target_rate", color="#d62728")
    ax.set_title("Recall Quality", fontsize=12, fontweight='bold')
    ax.set_xlabel("Turn")
    ax.set_ylabel("Score")
    ax.set_ylim(0.0, 1.0)
    ...
    # 3. Memory Growth - 记忆增长（双Y轴）
    ax = axes[1, 0]
    ax.plot(turns, memory_count, marker="o", linewidth=2.0, color="#1f77b4", label="total_memory")
    ax.plot(turns, hot_count, marker="o", linewidth=2.0, color="#2ca02c", label="hot_memory")
    ax.set_title("Memory Growth", fontsize=12, fontweight='bold')
    ax.set_xlabel("Turn")
    ...
    # 右Y轴 - 热记忆比例
    ax2 = ax.twinx()
    ax2.plot(turns, hot_ratio, marker="o", linewidth=2.0, color="#ff7f0e", label="hot_ratio")
    ...
    # 4. Compute Load Proxy - 计算负载代理（双Y轴）
    ax = axes[1, 1]
    ax.plot(turns, sim_ops_avg, marker="o", linewidth=2.0, label="avg_sim_ops", color="#1f77b4")
    ax.plot(turns, sim_ops_p95, marker="s", linewidth=2.0, label="p95_sim_ops", color="#ff7f0e")
    ax.set_title("Compute Load Proxy", fontsize=12, fontweight='bold')
    ax.set_xlabel("Turn")
    ax.set_ylabel("Similarity Ops / Turn")
    ...
    ax2.plot(turns, rescue_exec, linestyle="--", color="#d62728", linewidth=1.8, label="cold_rescue")
    ax2.plot(turns, preload_heated, linestyle="--", color="#9467bd", linewidth=1.8, label="preload_heated")
    ax2.set_ylabel("Event Count", color="#666666")
    ...
    # 总标题
    fig.suptitle(
        "Agent Memory Health Dashboard\n"
        f"turns={int(summary.get('turns', 0))}, merge_rate={summary.get('merge_rate', 0.0):.3f}, "
        f"gini_delta={summary.get('heat_gini_delta', 0.0):+.3f}, "
        f"hit_rate={summary.get('target_hit_rate', 0.0):.3f}",
        fontsize=14,
        fontweight='bold'
    )
    
    plt.savefig(output_path, dpi=max(72, dpi), bbox_inches="tight")
    plt.close()
...
def plot_results(result: Dict[str, object], output_path: str = "sim_plots.png", 
    ...
    Returns:
        保存的文件路径，如果 matplotlib 不可用则返回 None
    """
    if not HAS_MATPLOTLIB:
        print("Warning: matplotlib not available, skipping plot generation")
        return None
    ...
    # 提取数据
    turns = [s.get("turn", i) for i, s in enumerate(snapshots)]
    
    # 关键指标
    empty_query_rate = [s.get("empty_query_rate", 0) for s in snapshots]
    empty_target_rate = [s.get("empty_target_query_rate", 0) for s in snapshots]
    target_hit_rate = [s.get("target_hit_rate", 0) for s in snapshots]
    target_recall = [s.get("target_recall_recent", 0) for s in snapshots]
    
    # 系统指标
    heat_gini = [s.get("heat_gini", 0) for s in snapshots]
    hot_ratio = [s.get("hot_memory_ratio", 0) for s in snapshots]
    cluster_count = [s.get("cluster_count", 0) for s in snapshots]
    memory_count = [s.get("memory_count", 0) for s in snapshots]
    
    # 性能指标
    avg_sim_ops = [s.get("avg_sim_ops_total_per_turn", 0) for s in snapshots]
    p95_sim_ops = [s.get("p95_sim_ops_total_per_turn", 0) for s in snapshots]
    
    # 预加载指标
    preload_success = [s.get("preload_success_rate", 0) for s in snapshots]
    preload_heated = [s.get("preload_memories_heated", 0) for s in snapshots]
    
    # 创建图表
    fig, axes = plt.subplots(3, 3, figsize=figsize)
    fig.suptitle('Agent Memory Simulation Results', fontsize=14, fontweight='bold')
    
    # ========== 第一行：核心检索指标 ==========
    
    # 1. 空查询率
    ax = axes[0, 0]
    ax.plot(turns, empty_query_rate, 'b-o', linewidth=2, markersize=4, label='empty_query_rate')
    ax.plot(turns, empty_target_rate, 'r-s', linewidth=2, markersize=4, label='empty_target_rate')
    ax.set_xlabel('Turn')
    ax.set_ylabel('Rate')
    ax.set_title('Empty Query Rates')
    ax.legend()
    ax.grid(True, alpha=0.3)
    ...
    # 2. 目标命中率
    ax = axes[0, 1]
    ax.plot(turns, target_hit_rate, 'g-o', linewidth=2, markersize=4, label='target_hit_rate')
    ax.plot(turns, target_recall, 'm-s', linewidth=2, markersize=4, label='target_recall_recent')
    ax.set_xlabel('Turn')
    ax.set_ylabel('Rate')
    ax.set_title('Target Hit & Recall')
    ax.legend()
    ax.grid(True, alpha=0.3)
    ax.set_ylim(0, 1.05)
    ...
    # 3. 热度分布基尼系数
    ax = axes[0, 2]
    ax.plot(turns, heat_gini, 'orange', linewidth=2, marker='o', markersize=4)
    ax.axhline(y=summary.get('heat_gini_start', 0), color='green', linestyle='--', 
               label=f"start: {summary.get('heat_gini_start', 0):.3f}")
    ax.axhline(y=summary.get('heat_gini_end', 0), color='red', linestyle='--',
               label=f"end: {summary.get('heat_gini_end', 0):.3f}")
    ax.set_xlabel('Turn')
    ax.set_ylabel('Gini Coefficient')
    ax.set_title('Heat Distribution (Gini)')
    ax.legend()
    ax.grid(True, alpha=0.3)
    ...
    # 4. 记忆数量
    ax = axes[1, 0]
    ax.plot(turns, memory_count, 'b-o', linewidth=2, markersize=4, label='total_memory')
    ax.set_xlabel('Turn')
    ax.set_ylabel('Count')
    ax.set_title('Memory Count')
    ax.legend()
    ax.grid(True, alpha=0.3)
    
    # 5. 热记忆比例
    ax = axes[1, 1]
    ax.plot(turns, hot_ratio, 'purple', linewidth=2, marker='o', markersize=4)
    ax.set_xlabel('Turn')
    ax.set_ylabel('Ratio')
    ax.set_title('Hot Memory Ratio')
    ax.grid(True, alpha=0.3)
    ax.set_ylim(0, 1)
    
    # 6. 簇数量
    ax = axes[1, 2]
    ax.plot(turns, cluster_count, 'teal', linewidth=2, marker='o', markersize=4)
    ax.set_xlabel('Turn')
    ax.set_ylabel('Count')
    ...
    # 7. 计算开销
    ax = axes[2, 0]
    ax.plot(turns, avg_sim_ops, 'b-o', linewidth=2, markersize=4, label='avg_sim_ops')
    ax.plot(turns, p95_sim_ops, 'r-s', linewidth=2, markersize=4, label='p95_sim_ops')
    ax.set_xlabel('Turn')
    ax.set_ylabel('Operations')
    ax.set_title('Similarity Operations')
    ax.legend()
    ax.grid(True, alpha=0.3)
    
    # 8. 预加载成功率
    ax = axes[2, 1]
    ax.plot(turns, preload_success, 'green', linewidth=2, marker='o', markersize=4)
    ax.set_xlabel('Turn')
    ax.set_ylabel('Rate')
    ax.set_title('Preload Success Rate')
    ax.grid(True, alpha=0.3)
    ax.set_ylim(0, 1)
    
    # 9. 累计预热记忆数
    ax = axes[2, 2]
    ax.plot(turns, preload_heated, 'coral', linewidth=2, marker='o', markersize=4)
    ax.set_xlabel('Turn')
    ax.set_ylabel('Count')
    ax.set_title('Cumulative Preloaded Memories')
    ax.grid(True, alpha=0.3)
...
def plot_comparison(results: Dict[str, Dict], output_path: str = "comparison_plot.png",
                    figsize: Tuple[int, int] = (14, 10)) -> Optional[str]:
    """对比多个实验结果
    
    Args:
        results: {实验名称: result} 字典
        output_path: 输出路径
        figsize: 图表大小
    ...
    for i, (name, result) in enumerate(results.items()):
        snapshots = result.get("snapshots", [])
        turns = [s.get("turn", j) for j, s in enumerate(snapshots)]
        empty_rate = [s.get("empty_query_rate", 0) for s in snapshots]
        ax.plot(turns, empty_rate, color=colors[i % len(colors)], 
                marker=markers[i % len(markers)], linewidth=2, markersize=4, label=name)
    ax.set_xlabel('Turn')
    ax.set_ylabel('Rate')
    ax.set_title('Empty Query Rate')
    ...
    for i, (name, result) in enumerate(results.items()):
        snapshots = result.get("snapshots", [])
        turns = [s.get("turn", j) for j, s in enumerate(snapshots)]
        empty_target = [s.get("empty_target_query_rate", 0) for s in snapshots]
        ax.plot(turns, empty_target, color=colors[i % len(colors)],
                marker=markers[i % len(markers)], linewidth=2, markersize=4, label=name)
    ax.set_xlabel('Turn')
    ax.set_ylabel('Rate')
    ax.set_title('Empty Target Query Rate')
    ax.legend(fontsize=8)
    ax.grid(True, alpha=0.3)
    ...
    for i, (name, result) in enumerate(results.items()):
        snapshots = result.get("snapshots", [])
        turns = [s.get("turn", j) for j, s in enumerate(snapshots)]
        hit_rate = [s.get("target_hit_rate", 0) for s in snapshots]
        ax.plot(turns, hit_rate, color=colors[i % len(colors)],
                marker=markers[i % len(markers)], linewidth=2, markersize=4, label=name)
    ax.set_xlabel('Turn')
    ax.set_ylabel('Rate')
    ax.set_title('Target Hit Rate')
    ax.legend(fontsize=8)
    ax.grid(True, alpha=0.3)
    ax.set_ylim(0, 1.05)
    ...
    empty_rates = [results[n].get("summary", {}).get("empty_query_rate", 0) for n in names]
    target_rates = [results[n].get("summary", {}).get("empty_target_query_rate", 0) for n in names]
    hit_rates = [results[n].get("summary", {}).get("target_hit_rate", 0) for n in names]
    
    ax.bar(x - width, empty_rates, width, label='empty_query_rate', color='blue', alpha=0.7)
    ax.bar(x, target_rates, width, label='empty_target_rate', color='red', alpha=0.7)
    ax.bar(x + width, hit_rates, width, label='target_hit_rate', color='green', alpha=0.7)
...
def main():
    ...
    parser.add_argument("--output", type=str, default="sim_results.json", help="Output JSON file")
    parser.add_argument("--plot", type=str, default=None, help="Plot output file (e.g., sim_plots.png). If not specified, no plots generated.")
    parser.add_argument("--health-plot", type=str, default=None, help="Health dashboard output file (e.g., health_dashboard.png). Core health metrics.")
    parser.add_argument("--plot-size", type=str, default="16x12", help="Plot size as WxH (e.g., 16x12)")
    ...
    # 解析图表尺寸
    try:
        w, h = map(int, args.plot_size.lower().split('x'))
        figsize = (w, h)
    except:
        figsize = (16, 12)
    ...
    # 生成健康度仪表板（核心健康指标）
    if args.health_plot:
        health_path = args.health_plot if args.health_plot.endswith('.png') else args.health_plot + '.png'
        plot_health_dashboard(result, health_path, figsize=(16, 10))
    
    # 生成详细图表（9宫格）
    if args.plot:
        plot_path = args.plot if args.plot.endswith('.png') else args.plot + '.png'
        plot_results(result, plot_path, figsize)
